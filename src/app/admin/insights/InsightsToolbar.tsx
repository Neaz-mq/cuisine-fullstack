...
/**
 * Figma: "Search by Food Name..." (white, 50px, full width) + "All
 * Statuses ⌄" (white pill). Same build as the Menu page's MenuToolbar, so
 * both search boxes feel identical.
 *
 * Both filters apply to every item card on the page — Top Selling,
 * Slowest, Never Selling, Revenue by Category and the rest.
 */
export default function InsightsToolbar({ status }: { status: MenuStatusFilter }) {
    ...
    <div className="flex flex-col gap-3 min-[480px]:flex-row min-[480px]:items-center min-[480px]:gap-4 md:gap-6">
      <div className="relative h-[50px] min-w-0 flex-1">
        ...
        <input
          ...
          aria-label="Search insights by food name"
